...
void sendpkt3() {
    ...
    for(int i=0;i<4;i++) {
        if(i!=3 && i!=1) { // not to itself and not to 1
            tolayer2(pkt3[i]);
...
void calc_send_pkt3() {
    ...
    if (change == 1) { 
        sendpkt3();
        printf("Minimum cost updated at node 3: (%d  %d  %d  %d)\n", 
               min_cost_3[0], min_cost_3[1], min_cost_3[2], min_cost_3[3]);
    }
    else
...
void rtinit3() 
{
  ...
  printdt3(&dt3);
  calc_min_cost3();
  printf("Minimum cost at initialization from node 3: (%d  %d  %d  %d)\n",
         min_cost_3[0], min_cost_3[1], min_cost_3[2], min_cost_3[3]);
  sendpkt3();
}
